...
const clamp = (v, n = MAX_MESSAGE_CHARS) =>
  str(v).slice(0, n);
...
function providerName(v) {
  ...
  return s.includes('gemini') ? 'gemini' : 'gpt';
}
...
function getBrain(ownerId, provider, incomingHistory) {
  ...
  if (!brain.history.length && Array.isArray(incomingHistory)) {
...
async function withProviderQueue(provider, fn) {
  const previous = providerChains[provider];

  let release;

  providerChains[provider] = new Promise(resolve => {
    release = resolve;
  });
  ...
  try {
    ...
    const wait =
      minGap[provider] -
      (Date.now() - lastRequestAt[provider]);
...
async function fetchJson(url, options, label) {
  ...
  throw last || new Error(`${label}: namelum xeta`);
}
...
function buildSystemPrompt({
  ...
  const base = `Sən Roblox oyununda yaşayan ${ai} AI NPC-sən.
Sahibin ${ownerName}-dir.

Azərbaycan dilində, təbii və qısa danış.
User, GPT, Gemini, Assistant kimi başlıq yazma.
İstifadəçinin açıq əmrlərini oyun action-u ilə icra et.
Avtomatik gəzişmə yalnız istifadəçi istəyəndə olsun.
Normal hərəkət fiziki olmalıdır.
Teleport yalnız istifadəçi teleport istəyəndə.
...
Owner UserId: ${ownerUserId}.`;
  ...
  if (!actionMode) {
    return `${base}${context}
Bu adi söhbətdir; actions boş array olsun.`;
  }

  return `${base}${context}

Yalnız bu JSON formasında cavab ver:
{"reply":"...","actions":[{"type":"ACTION", ...}]}
...
WALK_TO:
{"type":"WALK_TO","position":[x,y,z],"distance":3}

TURN:
{"type":"TURN","direction":"LEFT|RIGHT","degrees":90}

MOVE:
{"type":"MOVE","direction":"forward|back|left|right","duration":3,"speed":8}
...
BUILD:
{"type":"BUILD","name":"House","description":"istifadəçinin dəqiq istəyi","parts":[{"shape":"Block","size":[4,1,4],"offset":[0,0,0],"material":"Plastic","color":[255,255,255],"anchored":true,"name":"Part"}]}

TOOLBOX:
{"type":"TOOLBOX","query":"house","count":1}

Tikinti üçün istifadəçinin dediyi konkret obyektə uyğun parts yarat.
"ev" deyilibsə maşın qurma.
"maşın" deyilibsə ev qurma.

Hazır model əvəzinə BUILD action-da hissələrdən qurmaq əsas üsuldur.
Toolbox yalnız istifadəçi açıq istəyəndə istifadə olunur.
...
Yalnız istifadəçi script istəyirsə script action qaytar.
Yeni script yarat.
Mövcud scripti özbaşına overwrite və ya fix etmə.
Silinmə yalnız istifadəçi açıq şəkildə silməyi istəyirsə.
...
Owner-only script tələb olunanda yalnız UserId ${ownerUserId} üçün işləməlidir.`;
}
...
function normalizeAction(a) {
  ...
  const x = {
    ...a,
    type: str(a.type).toUpperCase().trim()
  };
  ...
  if (Array.isArray(x.size)) {
    x.size = x.size
      .slice(0, 3)
      .map(v =>
        Math.max(0.1, Number(v) || 1)
      );
  }

  if (Array.isArray(x.offset)) {
    x.offset = x.offset
      .slice(0, 3)
      .map(v => Number(v) || 0);
  }
  ...
  if (x.count != null) {
    x.count = Math.max(
      1,
      Math.min(10, Number(x.count) || 1)
    );
  }

  if (x.assetId != null) {
    x.assetId = Number(x.assetId) || 0;
  }

  if (x.duration != null) {
    x.duration = Math.max(
      0.2,
      Math.min(120, Number(x.duration) || 3)
    );
  }
  ...
  if (x.degrees != null) {
    x.degrees = Math.max(
      1,
      Math.min(360, Number(x.degrees) || 90)
    );
  }
...
function normalizeModelOutput(raw) {
  ...
  if (p && typeof p === 'object') {
    ...
    if (!actions.length && p.action) {
      const one = normalizeAction({
        type: p.action
      });
...
async function callGPT({
  ...
  const system =
    buildSystemPrompt({
      ...
      ownerName,
      ownerUserId,
      world: actionMode ? world : null,
      assets: actionMode ? assets : [],
      actionMode
    });
...
function inferTarget(prompt, supplied) {
  const p = normalizeText(prompt);
  const s = normalizeText(supplied);

  if (
    s.includes('starterplayer') ||
    s.includes('localscript') ||
    s.includes('local script')
  ) {
    return 'StarterPlayerScripts';
  }

  if (
    p.includes('starterplayer') ||
    p.includes('localscript') ||
    p.includes('local script') ||
    p.includes('client script') ||
    p.includes('input')
  ) {
    return 'StarterPlayerScripts';
  }
...
async function createStudioScriptTask(body) {
  const prompt =
    clamp(
      body?.prompt ||
      body?.message ||
...
The script must affect ONLY that owner.
Never use loadstring, executor APIs, backdoors, arbitrary require IDs, or external URLs.
Do not overwrite other scripts.
Return ONLY source code, no markdown.`;
